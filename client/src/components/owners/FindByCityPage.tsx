...
interface IFindByCityPageProps {
  location: HistoryModule.Location;
}

interface IFindByCityPageState {
  owners?: IOwner[];
  filter?: string;
}

const getFilterFromLocation = (location) => {
  return location.query ? (location.query as any).city : null;
};

export default class FindByCityPage extends React.Component<IFindByCityPageProps, IFindByCityPageState> {
  ...
  componentDidMount() {
    ...
    if (typeof filter === 'string') {
      ...
      // i.e. city query param in uri was set
      this.fetchData(filter);
    }
  }

  componentWillReceiveProps(nextProps: IFindByCityPageProps) {
   ...
   * 
   * This method updates the URL with the entered city. The change of the URL
  ...
  fetchData(filter: string) {
    ...
    const requestUrl = url('api/owner2/list?city=' + query);
        ...
        <section>
          ...
          <form className='form-horizontal' action='javascript:void(0)'>
            <div className='form-group'>
              <div className='control-group' id='city'>
